(function (global) {
  ...
  class MatchClient extends EventTarget {
    ...
    emitError(error) {
      const messages = {
        insufficient_coins: 'Bu masa için yeterli Kantin Coin yok.',
        invalid_stake: 'Bu masanın giriş ücreti geçersiz. Lobiye dönüp yeniden seç.',
    ...
    connectSocial() {
      ...
      socket.addEventListener('message', event => {
        ...
        if (this.useWebSocketMatch && message.type === 'game:finished') {
          if (payload.coin) { this.lastSettlement={...payload.coin,matchId:payload.matchId}; this.profile = { ...this.profile, coins: payload.coin.balance }; this.emit('coins:updated', { ...payload.coin, matchId: payload.matchId }); }
    ...
    consume(payload, options = {}) {
      if (payload.wallet) global.KANTIN_ECONOMY?.applyWallet?.(payload.wallet);
      if (payload.coin && payload.match) {
        const coinKey=payload.match.matchId+':'+payload.coin.type;
        if(payload.coin.type!=='entry') this.lastSettlement={...payload.coin,matchId:payload.match.matchId};
        if(this.lastCoinKey!==coinKey){
          this.lastCoinKey=coinKey;
          this.profile={...this.profile,coins:payload.wallet?.balance??payload.coin.balance};
          this.emit('coins:updated',{...payload.coin,matchId:payload.match.matchId});
        }
      }
      if (payload.queue) {
        this.queued = true;
      ...
      if (payload.match && payload.gameState) {
        ...
        if (payload.result && this.finishedMatchId !== payload.match.matchId) {
          ...
          this.emit('game:finished', { matchId: payload.match.matchId, result: payload.result, coin: payload.coin || null });
        }
      }
    ...
    async poll() {
      if (!this.polling || this.pollBusy || this.actionPending) return this.schedulePoll();
      this.pollBusy = true;
      try {
        const revision=this.actionRevision||0;
        const payload = await this.request({ action: 'status' });
        if(!this.actionPending&&revision===(this.actionRevision||0))this.consume(payload, { resumed: Boolean(this.match) });
      } catch (error) {
    ...
    act(action, payload = {}) {
      ...
      this.actionPending=true;this.actionRevision=(this.actionRevision||0)+1;
      this.emit('game:pending',{action,payload});
      this.request(request).then(result => this.consume(result, { forceState: true })).catch(error => this.emitError(error)).finally(()=>{this.actionPending=false;this.emit('game:settled',{});});
    }
