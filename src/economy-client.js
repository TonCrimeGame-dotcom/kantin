(() => {
  ...
  function applyWallet(wallet, notify=true) {
    const version=Number(wallet?.version),balance=Number(wallet?.balance);
    if(!Number.isSafeInteger(version)||version<state.walletVersion||!Number.isSafeInteger(balance)||balance<0)return;
    const changed=state.balance!==balance||state.walletVersion!==version;
    state.balance=balance;state.walletVersion=version;
    if(changed&&notify)emit();
  }

  function applyEconomy(payload) {
    const wallet = payload?.wallet || {};
    applyWallet(wallet,false);
  ...
  async function refresh() {
    ...
    refreshPromise = Promise.all([rpc('kantin_my_economy'), rpc('kantin_rewarded_ad_state')])
      ...
      .catch(error => {
        state.status = 'error';
        if(state.balance===null)state.balance = positiveInteger(auth?.profile?.coins, 0);
