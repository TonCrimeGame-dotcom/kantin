...
(function (global) {
  ...
  const MODE_KOZ_MACA = 'kozMaca';
  const MODE_GOMMELI = 'gommeli';
  ...
  function normalizePlayers(players) {
    ...
    const normalized = players.map((p, index) => {
      ...
      // 0=A1, 1=B1, 2=A2, 3=B2
      const team = p.team || (index % 2 === 0 ? 'teamA' : 'teamB');
    ...
    // Takım kontrolü
    const a = normalized.filter(p => p.team === 'teamA').length;
    const b = normalized.filter(p => p.team === 'teamB').length;
  ...
  class BatakGame {
    constructor(options = {}) {
      ...
      const defaultPlayers = [
        { id: 'A1', username: 'Oyuncu A1', team: 'teamA' },
        { id: 'B1', username: 'Oyuncu B1', team: 'teamB' },
        { id: 'A2', username: 'Oyuncu A2', team: 'teamA' },
        { id: 'B2', username: 'Oyuncu B2', team: 'teamB' }
      ];
    ...
    completeTrick() {
      ...
      for (let i = 1; i < trick.length; i++) {
        ...
        if (currentCard.suit === winnerCard.suit) {
          ...
          if (rankOrder[currentCard.rank] > rankOrder[winnerCard.rank]) {
            winner = current;
          }
        }
      }
  ...
  // Always set global.BATAK for browser usage
  if (typeof window !== 'undefined') {
    window.BATAK = { BatakGame, MODE_KOZ_MACA, MODE_GOMMELI };
