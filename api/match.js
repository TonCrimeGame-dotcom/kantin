...
async function ticketFor(playerId, env) {
  const rows = await serviceRequest(`/rest/v1/matchmaking_tickets?select=player_id,username,mode,word_locale,stake,status,match_id,joined_at,updated_at&player_id=eq.${restFilter(playerId)}&limit=1`, {}, env);
  return Array.isArray(rows) ? rows[0] || null : null;
}

async function matchFor(matchId, env) {
  const rows = await serviceRequest(`/rest/v1/online_matches?select=id,mode,word_locale,players,state,turn_version,status,result,action_log,created_at,updated_at,finished_at,stake,coin_changes&id=eq.${restFilter(matchId)}&limit=1`, {}, env);
  return Array.isArray(rows) ? rows[0] || null : null;
}
...
function packetFor(row, playerId) {
  ...
  return {
    ...
    match: {
      ...
      mode: row.mode,
      stake: Number(row.stake)||0,
      pot: (Number(row.stake)||0)*row.players.length,
      wordLocale: row.word_locale,
      assignment,
      players: row.players
    },
    gameState: room.stateFor(playerId),
    coin: row.coin_changes?.[playerId] || null,
...
async function queuePacket(ticket, env) {
  ...
  const waiting = await serviceRequest(`/rest/v1/matchmaking_tickets?select=player_id&status=eq.waiting&stake=eq.${Number(ticket.stake)||0}&mode=eq.${restFilter(ticket.mode)}&word_locale=${localeFilter}&order=joined_at.asc,player_id.asc`, {}, env);
...
async function join(identity, body, env) {
  ...
  if (!Number.isSafeInteger(Number(body.stake ?? 0)) || Number(body.stake ?? 0)<0) throw new MatchApiError('invalid_stake',400);
...
async function leave(identity, env) {
  ...
  if (row?.status === 'playing' && assignmentFor(row, identity.playerId)) {
    const current = await initializeMatch(row, env);
    const room = roomFor(current);
    room.replaceWithBot(identity.playerId);
    const saved = await serviceRequest(`/rest/v1/online_matches?id=eq.${restFilter(row.id)}&turn_version=eq.${current.turn_version}&status=eq.playing&select=id`, {
      method: 'PATCH',
=======
'use strict';
...
>>>>>>> origin/main
      headers: { prefer: 'return=representation' },
      body: { players: room.players, state: room.fullState(), turn_version: current.turn_version + 1, updated_at: new Date().toISOString() }
    }, env);
    if (!saved?.length) throw new MatchApiError('state_conflict', 409, 'Oyun güncellendi. Çıkışı tekrar dene.');
<<<<<<< HEAD
  }
...
async function act(identity, body, env) {
  const [ticket, requestedRow] = await Promise.all([ticketFor(identity.playerId, env),body.matchId?matchFor(String(body.matchId), env):Promise.resolve(null)]);
  ...
  let row = body.matchId?requestedRow:await matchFor(ticket.match_id, env);
...
>>>>>>> origin/main
    if (payload.coin && /^[0-9a-f-]{36}$/i.test(identity.playerId)) {
      const wallets=await serviceRequest('/rest/v1/coin_wallets?select=balance,version&user_id=eq.'+restFilter(identity.playerId)+'&limit=1',{},env);
      payload.wallet=wallets?.[0]||null;
    }
<<<<<<< HEAD
