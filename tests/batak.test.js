...
test('Koz Maça puanlaması eksik, üç fazla ve El Almaz durumlarını uygular',()=>{const game=new BatakGame({mode:MODE_KOZ_MACA,players});game.state.bids={P1:3,P2:3,P3:2,P4:0};game.state.tricksWon={P1:3,P2:6,P3:1,P4:0};const result=game.finishGame();assert.deepEqual(result.scores,{P1:30,P2:-3,P3:-3,P4:10})});
...
test('Koz Maça açarın iki fazlasına kadar kabul eder, üç ve üzeri fazlada batırır',()=>{
 for(let bid=1;bid<=13;bid++)for(let won=0;won<=13;won++){
  ...
  const expected=won<bid||won-bid>=3?-bid:bid*10+(won-bid);
...
test('3 deyip 3 alan, 3 deyip 7 alan oyuncuyu yener',()=>{const game=new BatakGame({mode:MODE_KOZ_MACA,players});game.state.bids={P1:0,P2:2,P3:3,P4:3};game.state.tricksWon={P1:3,P2:0,P3:7,P4:3};game.finishGame();assert.deepEqual(game.state.scores,{P1:-10,P2:-3,P3:-3,P4:30});assert.deepEqual(game.state.winners,['P4']);assert.equal(game.state.winnerPlayerId,'P4')});

test('Koz Maça eksik ve üç fazla alanlar en yüksek ihale kadar batar',()=>{const game=new BatakGame({mode:MODE_KOZ_MACA,players});game.state.bids={P1:5,P2:2,P3:1,P4:2};game.state.tricksWon={P1:5,P2:1,P3:4,P4:3};assert.deepEqual(game.finishGame().scores,{P1:50,P2:-5,P3:-5,P4:21})});

test('Higher fulfilled bid wins: 4/4 scores 40 while 3/4 scores 31',()=>{const game=new BatakGame({mode:MODE_KOZ_MACA,players});game.state.bids={P1:3,P2:4,P3:2,P4:3};game.state.tricksWon={P1:2,P2:4,P3:3,P4:4};const result=game.finishGame();assert.deepEqual(result.scores,{P1:-4,P2:40,P3:21,P4:31});assert.deepEqual(result.winners,['P2']);assert.equal(result.winnerPlayerId,'P2');});
