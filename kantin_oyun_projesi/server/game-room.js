...
class GameRoom{
  ...
  clockKey(){if(this.status!=='playing'||this.mode==='upvp'||this.engine.state?.status!=='playing')return null;const s=this.engine.state;if(this.mode==='spvp'){if(!s.openingComplete)return null;if(s.pendingDouble)return`double:${s.turnNumber}:${s.pendingDouble.from}:${s.pendingDouble.to}:${s.pendingDouble.proposedValue}`;return`turn:${s.turnNumber}:${s.turn}`}if(this.mode.startsWith('pisti'))return s.pendingBluff?`bluff:${s.moveNumber}:${s.pendingBluff.responderId}`:`turn:${s.moveNumber}:${this.engine.getCurrentPlayer().id}`;if(this.mode.startsWith('okey'))return`turn:${s.turnNumber}:${this.engine.getCurrentPlayer().id}`;if(this.mode==='sozcukDuel')return`turn:${s.turnNumber}:${this.engine.current().id}`;return null}
  clockPlayer(){const s=this.engine.state;if(this.mode==='spvp')return s.pendingDouble?.to||s.turn;if(this.mode==='sozcukDuel')return this.engine.current().id;return this.engine.getCurrentPlayer().id}
  ...
  botCompleteTableTurn(player){if(this.mode.startsWith('pisti')){const hand=this.engine.getHand(player);if(hand.length)this.engine.playCard(player,hand[0].id);return}if(this.mode.startsWith('okey')){if(this.engine.state.phase==='draw')this.engine.drawFromStock(player);const hand=this.engine.state.hands[player];if(hand.length)this.engine.discard(player,hand[hand.length-1].id);return}if(this.mode==='sozcukDuel')this.engine.pass(player)}
