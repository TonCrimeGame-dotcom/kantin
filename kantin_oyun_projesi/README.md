...
- AdMob'da URL doğrulanırken test amaçlı `Custom data` alanına `kantin-admob-verification` yazılır; `User ID` boş bırakılabilir. Doğrulama başarılı olduktan sonra `Use verified URL` ve `Save` seçilir. Gerçek reklamda native köprü bu test değerinin yerine ödül oturumu UUID'sini gönderir.
- Vercel sunucu ortamında `SUPABASE_SERVICE_ROLE_KEY` veya yeni biçimdeki `SUPABASE_SECRET_KEY`, `REWARDED_AD_PROVIDER=admob` ve platforma göre `ADMOB_REWARDED_AD_UNIT_ID_ANDROID` / `ADMOB_REWARDED_AD_UNIT_ID_IOS` tanımlanır. Sunucu anahtarı hiçbir istemci paketine veya Git deposuna eklenmez.
- Android AdMob uygulama ve reklam birimi kimlikleri `mobile/admob.config.json` içinde tutulur. Native paket üretim dışındaki derlemelerde Google'ın test reklam birimini, üretimde ise Kantin reklam birimini kullanmalıdır.
