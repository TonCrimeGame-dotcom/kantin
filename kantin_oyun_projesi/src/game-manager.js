(function(global){
  ...
  class GameManager{
    ...
    start(mode,options={}){const config=MODES[mode];if(!config)throw new Error(`Bilinmeyen oyun modu: ${mode}`);this.mode=mode;
      ...
      else if(config.family==='okey'){if(!global.OKEY101?.Solo101Okey||!global.OKEY101?.Team101Okey)throw new Error('101 Okey oyun motoru yüklenemedi.');this.viewerId=config.engineMode==='solo'?'P1':'A1';this.game=config.engineMode==='solo'?new global.OKEY101.Solo101Okey():new global.OKEY101.Team101Okey()}
